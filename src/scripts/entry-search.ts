...
if (!root) {
  ...
  const getStatusPrefix = (query: string, totalMatches: number) => {
    if (query && activeTagLabel) {
      return `${totalMatches} ${totalMatches === 1 ? 'match' : 'matches'} under tag #${activeTagLabel}`;
    }
    if (query) {
      return totalMatches === 0 ? 'No results.' : `${totalMatches} ${totalMatches === 1 ? 'match' : 'matches'}`;
    }
    return '';
  };
  ...
  const updateStatusForMatches = (query: string, totalMatches: number, visibleMatches: number) => {
    ...
    if (visibleMatches === totalMatches) {
      setStatus(query && !activeTagKey ? `Found ${totalMatches} ${totalMatches === 1 ? 'match' : 'matches'}` : prefix);
      return;
    }
    if (visibleMatches === 0) {
      setStatus(`${prefix} (None on this page)`);
      return;
    }
    setStatus(`${prefix} (${visibleMatches}  on this page)`);
  };
  ...
  const setDegradedMode = () => {
    ...
    showAllItems();
    syncSections(false);
    setStatus('Search index failed to load. Search is disabled.');
  };
  ...
  const indexLoader = createJsonIndexLoader<IndexItem>({
    ...
    onPending: () => {
      setStatus('Loading search index...', { visible: false });
    },
    onResolved: (data) => {
      indexHay = new Map(
        data.map((item) => [
