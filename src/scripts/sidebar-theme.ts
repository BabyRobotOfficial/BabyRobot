...
const applyTheme = (theme: string) => {
  ...
  if (themeBtn) {
    ...
    // 🎯 REPLACED CHINESE LABELS WITH ENG STRINGS:
...
const setReaderDisabled = (disabled: boolean) => {
  ...
  if (disabled) {
    // 🎯 DISABLED NOTIFICATION STATE:
    readerBtn.setAttribute('title', 'Reader Mode (Article pages only)');
    readerBtn.setAttribute('aria-label', 'Reader Mode (Only available on essay pages)');
    readerBtn.tabIndex = -1;
  } else {
    // 🎯 STANDARD AVAILABLE STATE:
    readerBtn.setAttribute('title', 'Reader Mode');
    readerBtn.setAttribute('aria-label', 'Reader Mode');
...
const applyReader = (on: boolean) => {
  ...
  if (readerExit) {
    // 🎯 REPLACED '退出阅读' OVERRIDE STATE:
    readerExit.setAttribute('aria-label', 'Exit Reader Mode');
    readerExit.setAttribute('title', 'Exit Reader Mode');
  }
