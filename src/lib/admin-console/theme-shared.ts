...
export const ADMIN_HOME_INTRO_LINK_OPTIONS = [
  ...
  { id: 'essay', label: 'The Heart', href: '/essay/' },
  { id: 'bits', label: 'The Brain', href: '/bits/' },
  { id: 'memo', label: 'FAQ', href: '/memo/' },
...
// 🎯 UPGRADED: Whitelisted spotify, kofi, and substack into the dashboard system presets array
export const ADMIN_SOCIAL_PRESET_IDS = ['github', 'x', 'email', 'spotify', 'kofi', 'substack'] as const satisfies readonly SiteSocialPresetId[];

// 🎯 FIXED: Provided fallback form positioning weights for the new entries
export const ADMIN_SOCIAL_PRESET_ORDER_DEFAULT: Record<SiteSocialPresetOrderKey, number> = {
  github: 1,
  x: 2,
  email: 3,
  spotify: 4,
...
type SiteSocialPresetOrderKey = keyof SiteSocialPresetOrder;

export const ADMIN_SOCIAL_ORDER_MIN = 1;
export const ADMIN_SOCIAL_ORDER_MAX = ADMIN_SOCIAL_PRESET_IDS.length + ADMIN_SOCIAL_CUSTOM_LIMIT;export const ADMIN_NAV_ORDER_MIN = 1;
...
export const ADMIN_SOCIAL_ICON_KEYS = [
  ...
  'linkedin',
  'website',
  // 🎯 NEW: Allowed custom links to utilize these explicit asset tags
  'spotify',
  'kofi',
  'substack'
...
socialLinks: {
        ...
        // 🎯 NEW: Added string data cleaners for our newly registered database fields
        spotify: normalizeTrimmed(socialLinks.spotify) || null,
        kofi: normalizeTrimmed(socialLinks.kofi) || null,
        substack: normalizeTrimmed(socialLinks.substack) || null,
        presetOrder: {
          github: parseOrder(
          ...
          // 🎯 NEW: Attached numeric layout index trackers for ordering processing loops
          spotify: parseOrder(
            rawPresetOrder.spotify as string | number | null | undefined,
            ADMIN_SOCIAL_PRESET_ORDER_DEFAULT.spotify
          ),
          kofi: parseOrder(
            rawPresetOrder.kofi as string | number | null | undefined,
            ADMIN_SOCIAL_PRESET_ORDER_DEFAULT.kofi
          ),
          substack: parseOrder(
            rawPresetOrder.substack as string | number | null | undefined,
            ADMIN_SOCIAL_PRESET_ORDER_DEFAULT.substack
          )
        },
...
export const createAdminWritableThemeSettingsGroups = (
  ...
  site: {
    ...
    socialLinks: {
      ...
      // 🎯 NEW: Authorized writing these field links out to site.json file blocks
      spotify: settings.site.socialLinks.spotify,
      kofi: settings.site.socialLinks.kofi,
      substack: settings.site.socialLinks.substack,
      presetOrder: {
...
export const validateAdminThemeSettings = (
  ...
  if (!settings.site.defaultLocale) {
    ...
    pushIssue('site.defaultLocale', '默认语言格式无效（示例：en-US）');
  }
...
if (
    ...
    pushIssue('site.socialLinks.email', 'Email must be a valid email address'); // 🎯 Fixed: English
  }
  
  // 🎯 NEW: Appended URL validations for Spotify, Ko-fi, and Substack inputs
  if (settings.site.socialLinks?.spotify && !isAdminAllowedHttpsUrl(settings.site.socialLinks.spotify)) {
    pushIssue('site.socialLinks.spotify', 'Spotify link must be a valid https:// address');
  }
  if (settings.site.socialLinks?.kofi && !isAdminAllowedHttpsUrl(settings.site.socialLinks.kofi)) {
    pushIssue('site.socialLinks.kofi', 'Ko-fi link must be a valid https:// address');
  }
  if (settings.site.socialLinks?.substack && !isAdminAllowedHttpsUrl(settings.site.socialLinks.substack)) {
    pushIssue('site.socialLinks.substack', 'Substack link must be a valid https:// address');
  }
  ...
  ADMIN_SOCIAL_PRESET_IDS.forEach((id) => {
    // 🎯 FIXED: Translated dynamic label tags into clean English properties
    const rowLabel = id === 'github' ? 'GitHub' : id === 'x' ? 'X / Twitter' : id === 'email' ? 'Email' : id === 'spotify' ? 'Spotify' : id === 'kofi' ? 'Ko-fi' : 'Substack';
    const orderIssue = presetOrderIssues.get(id);
    if (orderIssue === 'range') {
      pushIssue(
        `site.socialLinks.presetOrder.${id}`,
        `${rowLabel} sort order placement must be an integer between ${ADMIN_SOCIAL_ORDER_MIN}-${ADMIN_SOCIAL_ORDER_MAX}` // 🎯 Fixed: English
      );
      return;
    }
    if (orderIssue === 'duplicate') {
      pushIssue(`site.socialLinks.presetOrder.${id}`, `Social link placement ordering cannot be duplicate: ${presetOrder[id]}`); // 🎯 Fixed: English
    }
  });
