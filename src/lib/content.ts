...
export async function getVisibleEssays(options: EssayQueryOptions = {}) {
  if (!shouldUseDefaultEssayCache(options.includeDraft)) {
    ...
    return essays.filter((entry: EssayEntry) => !isReservedSlug(getEssaySlug(entry)));
  }

  visibleEssaysPromise ??= getSortedEssays().then((essays: EssayEntry[]) =>
    essays.filter((entry: EssayEntry) => !isReservedSlug(getEssaySlug(entry)))
  );
...
export async function getArchiveEssays(options: EssayQueryOptions = {}) {
  if (!shouldUseDefaultEssayCache(options.includeDraft)) {
    const essays = await getSortedEssays(options);
    return essays.filter((entry: EssayEntry) => entry.data.archive !== false && !isReservedSlug(getEssaySlug(entry)));
  }

  archiveEssaysPromise ??= getSortedEssays().then((essays: EssayEntry[]) =>
    essays.filter((entry: EssayEntry) => entry.data.archive !== false && !isReservedSlug(getEssaySlug(entry)))
  );
...
export async function getVisibleEssayRouteEntries(options: EssayQueryOptions = {}) {
  // 1. Grab every single visible essay page
  const essays = await getVisibleEssays(options);

  // 2. Map over ALL of them so they safely pass to the router
  return essays.map((entry: EssayEntry, index: number) => ({
    slug: getEssaySlug(entry),
    entry,
    // Keep these basic fallbacks intact
